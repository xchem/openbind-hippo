...
for threshold in [85,90,95]:
    ...
    for hit_id, analogues in similar.items():
        ...
        analogues.add_tag(f"openbind_xx01zvns2b_c2_hit_analogues_{threshold}percent")
        analogues.add_tag(f"C{hit.id} {threshold}percent analogue")

animal.db.close()
refdb.db.close()

# sb.sh --job-name xx01zvns2b_c02c_hit_analogues --exclusive --no-requeue $HOME2/slurm/run_python.sh c02c_hit_analogues.py
